...
import { Plus, Trash2, Info, Copy, Check, Link2 } from "lucide-react"
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
export function GeneralContextForm({ client, isPublicMode = false, editToken, readOnly = false }: Props) {
  ...
  const loadContext = async () => {
    ...
    try {
      let data: ClientContext
      if (isPublicMode && editToken) {
        data = await api.getPublicContext(editToken)
      } else {
        data = await api.getContext(client.id)
      }
      setContext(data)
  ...
  const handleSave = async () => {
    ...
    try {
      if (isPublicMode && editToken) {
        await api.savePublicContext(editToken, context)
      } else {
  ...
  const handleGenerateEditToken = async () => {
    ...
    try {
      const updated = await api.generateContextEditToken(client.id)
      setContext(prev => ({ ...prev, edit_token: updated.edit_token }))
    } catch (e: any) {
      setError(e.message)
    } finally {
  ...
  const handleCopyEditLink = () => {
    if (context.edit_token) {
      const url = `${window.location.origin}/public/context/${context.edit_token}`
  ...
  return (
    <div className="space-y-6">
      ...
      {!isPublicMode && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>Share this link to allow external users to edit this context</CardDescription>
          </CardHeader>
          <CardContent>
            {context.edit_token ? (
              <div className="flex items-center gap-2">
                <Input
                  readOnly
                  value={`${typeof window !== 'undefined' ? window.location.origin : ''}/public/context/${context.edit_token}`}
      ...
      {isNewClient && !fetching && !isPublicMode && (
        <Alert>
